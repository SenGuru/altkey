...
from app.providers import chatgpt, claude_oauth
# Cookie-era providers removed; only OAuth providers remain.
# Gemini parked — code preserved at app/providers/gemini.py, disconnected from routing.


def test_for_model_routing_claude():
    assert for_model("claude-sonnet-4-5") is claude_oauth
    assert for_model("claude-opus-4-5") is claude_oauth
    assert for_model("claude-3-5-haiku-20241022") is claude_oauth
